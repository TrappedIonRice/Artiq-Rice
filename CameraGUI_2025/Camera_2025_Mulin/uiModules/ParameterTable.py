...
class Parameter(QtCore.QObject):
    ...
    Args:
        ...
        """
    """Class for parameters used in the parameter table."""
    changed = QtCore.pyqtSignal(object) #10/06/2025 by MZ
    ...
    def __init__(self, name, dataType, value, key=None, choices=None, text=None, tooltip=''):
        super().__init__()  # 10/06/2025 by MZ
    ...
    # Added 10/6/2025 by MZ for the load of parameters from json file to the camera GUI window
    def update(self, value):
        """Programmatically updates the parameter's value."""
        self.value = value
        # We emit the 'changed' signal here as well. This isn't strictly
        # necessary for the load to work, but it's good practice.
...
class ParameterTableModel(QtCore.QAbstractTableModel):
    ...
    def setValue(self, index, value):
        ...
        # Emit our new 'changed' signal from the parameter object itself
        parameter.changed.emit(parameter.value)

        self.valueChanged.emit(parameter)  # This existing line is for other parts of the program
        return True
    # 10/06/2025 by MZ in order to save the camera GUI parameter to the JSON file
