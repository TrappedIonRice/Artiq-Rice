...
class runScan(Fragment):
    ...
    def build_fragment(self):
        ...
        self.setattr_device("urukul1_ch3")  # Raman 1; B2

        self.setattr_device("urukul2_cpld")  # Necessary for clock sync
        self.setattr_device("urukul2_ch0") # Raman 1 ch1
        self.setattr_device("urukul2_ch1") # Raman 1; A16
        self.setattr_device("urukul2_ch2") # OP
        # self.setattr_device("urukul2_ch2")  # 26/06/30: switched to OP; was RR lock
    ...
    def ON(self,Frequency435,Amplitude435,Time435,Attenuation_435,choice435, doppler_freq,doppler_amp,doppler_time,
           ...
           freq_935,amp_935,
           OP_freq,OP_amp, OP_time,
           LOP_freq, LOP_amp, LOP_time,
           ...
           FrequencyRaman1, AmplitudeRaman1, FrequencyRamanB2, AmplitudeRamanB2, FrequencyRamanA16, AmplitudeRamanA16,
           Raman_time, LighShiftFactor, GlobalSidebandAmpScale, Bz,
           RamseyFrequency435, RamseyAmplitude435, PiBy2Time435_1,PiBy2Time435_2,
           newEndcapX, newAllY, newAllZ, endcapAvg_V, piezoR1H, piezoR1V ,piezoR2H ,piezoR2V ,
           num_repeat, iterScan, checkLineTrigger):
        ...
        # newX = newEndcapX - self.originalEndcapX # old regular endcapX scan
        # # newX = newEndcapX - self.originalEndcapX  + 0.0005*(-1)**(iterScan) # new X push for compensation.
        # newY = newAllY - self.originalAllY
        # newZ = newAllZ - self.originalAllZ

        # adding up combinations, but in absolute terms
        # newX = newEndcapX   # old regular endcapX scan; the endcapX in the scan is added to that in the dataset
        newX = newEndcapX - self.originalEndcapX  #+ 0.0005*(-1)**(iterScan) # new X push for compensation.
        newY = newAllY - self.originalAllY
        newZ = newAllZ - self.originalAllZ
        ...
        # delay(1*ms) # 26/06/30; Guido suggested that we give the filters some time to relax
        ...
        # self.zotino0.load()
        # delay(2 * ms)
        # self.zotino0.write_dac(26, piezoR2H)  # 26/07/13 gt: 26 was used for H of R2, now used for OP switch
        # self.zotino0.load()
        # delay(2 * ms)
        ...
        if iterScan==0:
            ...
            # 355 Raman 2; also shine for 1 s at beginning of each scan
            # self.ttl6.output()
            # self.ttl6.on()
            # delay(1000*ms)
            # self.ttl6.off()
            ...
            # self.urukul1_ch3.sw.off()
            self.urukul2_ch2.sw.off()
            # self.zotino0.write_dac(26, 5.0)  # 26/07/13 gt: turning off GOP switch
            # self.zotino0.load()
        ...
        with self.core_dma.record("seq"):
            ...
            # self.urukul0_ch2.sw.on()
            self.ttl6.on() # Raman 2, global, on
            ...
            # necessary to reduce doppler leakage into OP time
            # self.urukul0_ch1.set(frequency=doppler_freq, amplitude=0.001, phase_mode=2)
            # self.urukul0_ch1.set_att(30 * dB)
            self.urukul0_ch1.sw.off()
            # self.urukul0_ch2.sw.off()
            self.ttl6.off() # Raman 2, global, off
            delay(0.5*ms)
            ...
            #     self.urukul2_ch2.set(frequency=OP_freq, amplitude=OP_amp*(step*1.0)/STEPS, phase_mode=2)
            #     self.urukul0_ch1.sw.on()  # can't use dictionary under kernel
            #     self.urukul0_ch2.sw.on()
            #     # self.urukul1_ch3.sw.on()
            ...
            if SBCTime>0.1*us:
                ...
                #     # self.ttl6.off()
                #     self.urukul2_ch2.sw.on()
                #     delay(0.1* ms) # prev 0.03ms need strong OP power
                #     self.urukul2_ch2.sw.off()
                # # #
                # # #
                # # # # # # Inner 1
                ...
                #CSBC LOP
                # self.urukul2_ch0.set(frequency=SBCFrequency355_2, amplitude=SBCAmplitude355_2, phase_mode=2)
                self.urukul1_ch3.set(frequency=SBCFrequency355_2, amplitude=SBCAmplitude355_2, phase_mode=2)
                ...
                # CSBC Raman
                # self.urukul2_ch2.set(frequency=OP_freq, amplitude=SBCAmplitude935, phase_mode=2)
                # self.urukul2_ch0.sw.on()
                # self.ttl6.on()
                # self.urukul2_ch2.sw.on()
                # delay(SBCTime)
            ...
            #self.urukul0_ch2.set_att(0 * dB)
            if OP_time>0.01*us:
                self.urukul2_ch2.set(frequency=OP_freq*0+205*MHz, amplitude=OP_amp, phase_mode=2)
            ...
            # # (26/06/26): turning off det and doppler, op explicitly
            # self.urukul0_ch1.set_att(30 * dB) #dopp
            # self.urukul0_ch1.sw.off()
            # self.urukul0_ch3.set_att(30 * dB) # det
            # self.urukul0_ch3.sw.off()
            # self.urukul2_ch2.set_att(30 * dB) #OP
            # self.urukul2_ch2.sw.off()
            # delay(1*ms)

            self.urukul1_ch1.set(frequency=198.682*MHz, amplitude=LOP_amp, phase_mode=2)
            self.urukul1_ch1.set_att(0 * dB)
            ...
            if RamseyCheck==True:
                ...
                # # First pi/2 pulse
                self.urukul1_ch2.set_att(0 * dB)
                self.urukul1_ch2.set(frequency=RamseyFrequency435, phase = 0.0, amplitude=RamseyAmplitude435, phase_mode=2)
                ...
                #
                # self.endcapX(-0.177) # extra displacement from exisitng voltage config.
                # # self.allY(0.0)
                # # self.allZ(0.0)
                # for i in range(12):
                #     ind = self.DCElectrodeMapping[i]
                ...
                # #
                # delay_mu(1)
                # # self.urukul1_ch1.set(frequency=LOP_freq, amplitude=0.0001, phase_mode=2)
                # self.urukul1_ch1.set_att(30 * dB)
                # self.urukul1_ch1.sw.off()
                # delay(0.05 * ms)
                ...
                #
                # self.urukul1_ch3.set(frequency=SBCFrequency355_2, amplitude=SBCAmplitude355_2, phase_mode=2)
                # self.urukul1_ch1.set(frequency=LOP_freq+3.159*MHz, amplitude=LOP_amp, phase_mode=2)
                # self.urukul1_ch3.sw.on()
                # self.ttl6.on()
                # self.urukul1_ch1.sw.on()
                # delay(SBCTime)
                ...
                # #RamanA16
                #
                # self.urukul2_ch1.set(frequency=SBCFrequency355_2, amplitude=SBCAmplitude355_2, phase_mode=2)
                # self.urukul1_ch1.set(frequency=LOP_freq-3.159*MHz, amplitude=LOP_amp, phase_mode=2)
                # self.urukul2_ch1.sw.on()
                # self.ttl6.on()
                # self.urukul1_ch1.sw.on()
                ...
                self.urukul1_ch2.set(frequency=RamseyFrequency435, phase = phase2, amplitude=RamseyAmplitude435, phase_mode=2)
                # self.urukul1_ch2.set(frequency=MW_freq, phase = phase2, amplitude=RamseyAmplitude435, phase_mode=2)
                # self.urukul1_ch2.set_att(0 * dB)
                self.urukul1_ch2.sw.on()
                self.urukul1_ch2.set_att(0 * dB)
                ...
                #
                # self.urukul2_ch0.sw.off() # Raman1
                # self.ttl6.off()
                #
                ...
                # # self.urukul2_ch0.set_att(0 * dB)
                # # ### extra
                # # self.urukul2_ch1.set(frequency=FrequencyRaman2 , phase=0.0,
                ...
                # #
                # #     #prev used phase= n*pi/2.0
                # #     self.urukul2_ch0.set(frequency=RamseyFrequency435, phase=np.pi*n/2.0,
                # #                          amplitude=RamseyAmplitude435, phase_mode=2)
                # #     # self.urukul1_ch2.set(frequency=MW_freq, amplitude=RamseyAmplitude435, phase_mode=2)
                # #     self.urukul2_ch0.set_att(0 * dB)
                # #     self.urukul2_ch0.sw.on()
            ...
            # MW interaction
            # self.urukul2_ch2.set(frequency=OP_freq, amplitude=0.05, phase_mode=2)

            if MW_time>0.01*us and not RamseyCheck:
                ...
                ## after MW pi on the clock, wait, then apply a pi on the zeeman
                # self.urukul2_ch2.set(frequency=235*MHz, amplitude=0.00001, phase_mode=2)# OP
                # self.urukul2_ch2.set_att(30 * dB)  # OP
                # self.urukul2_ch2.sw.off()
                #
                # self.urukul0_ch3.set(frequency=235*MHz, amplitude=0.00001, phase_mode=2)# det
                # self.urukul0_ch3.set_att(30 * dB)  # det
                # self.urukul0_ch3.sw.off()
                ...
                # #
                # # #### pi on zeeman
                # self.urukul1_ch2.set(frequency=148.8154*MHz, amplitude=MW_amp, phase_mode=2)
                # # self.urukul1_ch2.set_att(0 * dB)
                # self.urukul1_ch2.set_att(0 * dB)
                # self.urukul1_ch2.sw.on()
                # # self.ttl5.on()
            ...
            #Raman
            if Raman_time > 0.01 * us and not RamseyCheck:
                ...
                self.urukul2_ch0.set(frequency=FrequencyRaman1, amplitude=AmplitudeRaman1, phase_mode=2)
                self.urukul2_ch0.set_att(0 * dB)
                self.urukul2_ch0.sw.on()# Raman 1; B1

                self.urukul1_ch3.set(frequency=FrequencyRamanB2, amplitude=AmplitudeRamanB2, phase_mode=2)
                # self.urukul1_ch3.set(frequency=FrequencyRamanB2, amplitude=0.05, phase_mode=2)

                self.urukul1_ch3.set_att(0 * dB)
                # self.urukul1_ch3.sw.on()  # Raman 1; B2

                self.urukul2_ch1.set(frequency=FrequencyRamanA16, amplitude=AmplitudeRamanA16, phase_mode=2)
                # self.urukul2_ch1.set(frequency=FrequencyRamanA16, amplitude=0.04, phase_mode=2)

                self.urukul2_ch1.set_att(0 * dB)
                # self.urukul2_ch1.sw.on()  # Raman 1; A16

                self.ttl6.on() # Raman 2
                delay(0.3*us) # AOM delay
                delay(Raman_time)
                self.urukul2_ch0.sw.off() # Raman 1; B1
                self.urukul1_ch3.sw.off()  # Raman 1; B2
                self.urukul2_ch1.sw.off()  # Raman 1; A16
                self.ttl6.off() # Raman 25*us
                ...
                #     self.urukul2_ch0.set_att(0 * dB)
                #     self.urukul2_ch1.set(frequency=FrequencyRaman2, phase= 0.0, amplitude=AmplitudeRaman2*np.sin(np.pi/2.0*(n+1)/20.0)**2, phase_mode=2)
                #     self.urukul2_ch1.set_att(0 * dB)
                #     self.urukul2_ch1.sw.on()# Raman 1,ch2
                #     self.urukul2_ch0.sw.on()  # Raman 1
                #     self.ttl6.on()  # Raman 2
                ...
                # self.urukul2_ch0.set(frequency=FrequencyRaman1 + Bz, phase=0.0,
                #                      amplitude=AmplitudeRaman1 * GlobalSidebandAmpScale, phase_mode=2)
                # self.urukul2_ch0.set_att(0 * dB)
                ...
                # self.urukul2_ch1.set(frequency=FrequencyRaman2 + Bz, phase=phase2,
                #                      amplitude=AmplitudeRaman2 * LighShiftFactor * GlobalSidebandAmpScale, phase_mode=2)
                # self.urukul2_ch1.set_att(0 * dB)
                # self.urukul2_ch0.sw.on()# Raman 1
                # self.urukul2_ch1.sw.on()# Raman 1,ch2
                # self.ttl6.on() # Raman 2
           ...
           # delay(wait_time)
            ...
            if det_time>0.01*us:
                ...
                #self.urukul2_ch2.set_att(30 * dB)
                # self.urukul2_ch2.set(frequency=OP_freq, amplitude=det_amp, phase_mode=2)
                #
                # self.urukul2_ch2.sw.off()
            ...
            # self.ttl5.off()
            # continue Doppler+935+ all repumpers
...
class executeScan(ExpFragment):
    ...
    def extract_dataset_defaults(self):
        ...
        self.default_freqLOP = self.get_dataset("LOP.Frequency")
        self.default_ampLOP = self.get_dataset("LOP.Amp")
        self.default_timeLOP = self.get_dataset("LOP.Time(ms)") * ms
        ...
        self.default_endcapAvg = self.get_dataset("Experiment_config.endcap_avg")
    ...
    def build_fragment(self):
        ...
        self.setattr_param("freqLOP", FloatParam, "LOP frequency", unit="MHz", default=self.default_freqLOP)
        self.setattr_param("ampLOP", FloatParam, "LOP amplitude", unit="", default=self.default_ampLOP,
                           max=0.8)
        self.setattr_param("timeLOP", FloatParam, "LOP time", unit="ms", min=0.00001 * ms,
                           default=self.default_timeLOP)
        ...
        self.setattr_param("Frequency355_RamanB2", FloatParam, "Frequency 355_RamanB2", unit="MHz",
                           default=self.default_RamanB2_freq)  # changed min to 1 to avoid fit issue when 0
        self.setattr_param("Amplitude355_RamanB2", FloatParam, "Amplitude 355_RamanB2 ", unit="",
                           default=self.default_RamanB2_amp, min=0.00,
                           max=0.8)

        self.setattr_param("Frequency355_RamanA16", FloatParam, "Frequency 355_RamanA16", unit="MHz",
                           default=self.default_RamanA16_freq)  # changed min to 1 to avoid fit issue when 0
        self.setattr_param("Amplitude355_RamanA16", FloatParam, "Amplitude 355_RamanA16 ", unit="",
                           default=self.default_RamanA16_amp, min=0.00,
                           max=0.8)

        self.setattr_param("RamanTime", FloatParam, "Raman time 355", unit="ms", default=self.default_Raman_time,
                           min=0.00001 * ms)

        self.setattr_param("LighShiftFactor", FloatParam, "Light Shift Factor (BSB) ", unit="",
                           default=1.0, min=0.00, max=2.0)
        self.setattr_param("GlobalSidebandAmpScale", FloatParam, "Global sideband scaling ", unit="",
                           default=1.0, min=0.00, max=2.0)
        self.setattr_param("Bz", FloatParam, "Bz", unit="kHz",
                           default=0.0*kHz)
        ...
        self.setattr_param("endcapAverage", FloatParam, "Endcap Avg ", unit="", default=self.default_endcapAvg)
        ...
        self.setattr_param("checkLineTrigger", BoolParam, "Line Trigger", default=False)
   ...
   #     self.modpreptimeOP=0.00001*ms*0
        ...
        if self.checkCameraDetection.get():
            ...
            self.send_datapacket.update({'Experiment exposure time': {"value":self.DetTime369.get()/ms , "unit":"ms" }}) # in ms
            self.set_dataset('Camera.x', json.dumps(self.scan_x_data['x']), persist=True)
            self.cameraCOMM_prescan()

        #setting time stamps for the experiment
        self.set_dataset("timestamp_arr", [], broadcast=True, archive=True, persist=True)
    ...
    def run_once(self):
        ...
        self.core.reset()
        # self.urukul2_ch2.sw.on()  # RR lock
        # self.urukul2_ch3.sw.on()  # 369 ULE

        self.uninterrupted_processes()

        # recording timestamp for each scan point.
        T=now_mu()
        self.append_to_dataset("timestamp_arr", T)
        ...
        self.runObj.ON(self.Frequency435.get(),self.Amplitude435.get(),self.Time435.get(),self.attenuation_435_1, self.choice435.get(),\
                    ...
                    self.det_freq,self.det_amp,self.DetTime369.get(), self.checkCameraDetection.get(),self.checkGlobalCoolingShot.get(),self.cameraCoolingShotTime, \
                    self.freq_935,self.amp_935, \
                    self.prepfreqOP.get(), self.prepampOP.get(), self.preptimeOP.get(),
                    self.freqLOP.get(), self.ampLOP.get(), self.timeLOP.get(),
                    self.FrequencyMW.get(), self.AmplitudeMW.get(), self.TimeMW.get(), \
                    self.SBCFrequency355_1.get(),self.SBCAmplitude355_1.get(),self.SBCFrequency355_2.get(),self.SBCAmplitude355_2.get(), self.modSBCtime, self.SBCAmplitude935.get(),\
                    ...
                    self.Frequency355_Raman1.get(),self.Amplitude355_Raman1.get() , self.Frequency355_RamanB2.get(), self.Amplitude355_RamanB2.get(), \
                    self.Frequency355_RamanA16.get(), self.Amplitude355_RamanA16.get(), \
                    self.RamanTime.get(), self.LighShiftFactor.get(), self.GlobalSidebandAmpScale.get(), self.Bz.get(),\
                    self.RamseyFrequency435mod, self.RamseyAmplitude435, self.PiBy2Time435_1mod, self.PiBy2Time435_2mod,\
                    self.endcapX.get(),self.allY.get(), self.allZ.get(), self.endcapAverage.get(), self.piezoR1H.get(), self.piezoR1V.get(), self.piezoR2H.get(), self.piezoR2V.get(),\
                    self.num_repeat, self.iter, self.checkLineTrigger.get() ) #calls ON function in runScan fragment
        ...
        #                self.WaitTime.get(), self.Ramseycheck.get(), self.Phase1.get(), self.Phase2.get(), \
        #                self.frequency355switch, self.amplitude355switch, self.attenuation355switch, \
        #                self.Frequency355_Raman1.get(), self.Amplitude355_Raman1.get(), self.Frequency355_RamanB2.get(),
        #                self.Amplitude355_RamanB2.get(), \
        #                self.Frequency355_RamanA16.get(), self.Amplitude355_RamanA16.get(), \
        #                self.RamanTime.get(), self.LighShiftFactor.get(), self.GlobalSidebandAmpScale.get(),
        #                self.Bz.get(), \
        #                self.RamseyFrequency435mod, self.RamseyAmplitude435, self.PiBy2Time435_1mod,
        #                self.PiBy2Time435_2mod, \
        #                self.endcapX.get(), self.allY.get(), self.allZ.get(), self.endcapAverage.get(),
        #                self.piezoR1H.get(), self.piezoR1V.get(), self.piezoR2H.get(), self.piezoR2V.get(), \
        #                self.num_repeat, self.iter, self.checkLineTrigger.get())  # calls ON function in runScan fragment
    ...
    def host_cleanup(self):
        ...
        # by default archive=True for all datasets so it should appear in expid, BUT IT DOESN'T SO SAVE IT EXPLICITLY
