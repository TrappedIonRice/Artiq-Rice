...
class BarebonesArtiqScanV2(EnvExperiment):
        ...
        # 2. Check for Standard Argument Scans
            ...
            for name in getattr(self, "scannable_names", []):
                ...
                if hasattr(arg, "value"):
                    # Case A: Fixed Value (NoScan)
                    setattr(self, name, arg.value)
                else:
                    # Case B: Scanning Value
                    try:
    ...
    def _setup_plotting(self):
        ...
        # Format plot arrays appropriately based on units
    ...
    def _build_kernel_arguments(self):
        ...
        if self.awg_enabled:
            awg_map = {"AWG.ch1.T0": "RamanTime", "AWG.ch2.T0": "RamanTime", "AWG.ch3.T0": "RamanTime",
        ...
        for idx, name in enumerate(self.on_params):
            ...
            if name in overrides:
                ...
                if hasattr(self, gui_attr_name):
                    ...
                    raw_val = 0.0
                    print(f"[WARNING] Missing parameter on self: {name} (also checked {gui_attr_name})")
        ...
        # Generate indices for kernel indexing
        for idx, name in enumerate(self.on_params):
    ...
    def ON(self, Frequency435, Amplitude435, Time435, Attenuation_435, choice435, doppler_freq, doppler_amp, doppler_time,
           ...
           SBCcheck, SBCFrequency355_1, SBCAmplitude355_1, SBCFrequency355_2, SBCAmplitude355_2, SBCTime, SBCAmplitude935,
           ClearoutPower935, ClearoutTime935,
           prepfreq435, preptime,
        ...
        self.zotino0.write_dac(24, piezoR1H)  # PZ H indiv
        self.zotino0.write_dac(25, piezoR1V)  # PZ V indiv
        # self.zotino0.write_dac(26, piezoR2H)  # now used for OP switch
        self.zotino0.write_dac(27, piezoR2V)  # PZ V glo
        self.zotino0.load()
        delay(2 * ms)
    ...
    # 26/07/17 gt: for descriptive indeces in defaults
    @kernel
    def krun(self, scan_vals, defaults, scan_idx, iter_idx):
        ...
        try:
            ...
            for i in range(len(scan_vals)):
                ...
                # FIX 1: Update the active scan parameter in the defaults array for this step
                if scan_idx != -1:
                    defaults[scan_idx] = scan_vals[i]

                # FIX 2: Update the current iteration counter in the defaults array
                if iter_idx != -1:
                    defaults[iter_idx] = float(i)

                # Your diagnostic print (now the hardware will actually use this value)
                print("Scan value in kernel: ", scan_vals[i])
                ...
                if defaults[self.idx_checkAllZ_calib] > 0.5:
                    ...
                    if i % n == 0:  # do calibration every nth scan point
                        ...
                        for j in range(self.AllZ_calib_num_pts):
                            ...
                            defaults[self.idx_allZ] = self.allZ_calib_array[j]
                            ...
                            self.ON(
                                ...
                                defaults[self.idx_SBCcheck] > 0.5, defaults[self.idx_SBCFrequency355_1], defaults[self.idx_SBCAmplitude355_1],
                                ...
                                defaults[self.idx_EnableAWG] > 0.5,
                ...
                if awg_enabled and is_live_mode:
                    self.load_awg_step_rpc(i, len(scan_vals), 0)
                ...
                # --- 4. Call ON (Physics) ---
                self.ON(
                    defaults[self.idx_Frequency435], defaults[self.idx_Amplitude435], defaults[self.idx_Time435],
                    defaults[self.idx_attenuation_435_1], int(defaults[self.idx_choice435channel_1_2]),
                    defaults[self.idx_doppler_freq], defaults[self.idx_doppler_amp], defaults[self.idx_doppler_time],
                    defaults[self.idx_det_freq], defaults[self.idx_det_amp], defaults[self.idx_DetTime369],
                    defaults[self.idx_checkCameraDetection] > 0.5, defaults[self.idx_checkGlobalCoolingShot] > 0.5,
                    defaults[self.idx_cameraCoolingShotTime],
                    defaults[self.idx_freq_935], defaults[self.idx_amp_935],
                    defaults[self.idx_prepfreqOP], defaults[self.idx_prepampOP], defaults[self.idx_preptimeOP],
                    defaults[self.idx_prepfreqLOP], defaults[self.idx_prepampLOP], defaults[self.idx_preptimeLOP],
                    defaults[self.idx_FrequencyMW], defaults[self.idx_AmplitudeMW], defaults[self.idx_TimeMW],
                    defaults[self.idx_SBCcheck] > 0.5, defaults[self.idx_SBCFrequency355_1], defaults[self.idx_SBCAmplitude355_1],
                    defaults[self.idx_SBCFrequency355_2], defaults[self.idx_SBCAmplitude355_2],
                    defaults[self.idx_SBCTime], defaults[self.idx_SBCAmplitude935],
                    defaults[self.idx_ClearoutPower935], defaults[self.idx_ClearoutTime935],
                    defaults[self.idx_prepfreq435], defaults[self.idx_preptime],
                    defaults[self.idx_WaitTime], defaults[self.idx_Ramseycheck] > 0.5, defaults[self.idx_Phase1],
                    defaults[self.idx_Phase2],
                    defaults[self.idx_EnableAWG] > 0.5,
                    defaults[self.idx_B1check] > 0.5, defaults[self.idx_Frequency355_Raman1],
                    defaults[self.idx_Amplitude355_Raman1],
