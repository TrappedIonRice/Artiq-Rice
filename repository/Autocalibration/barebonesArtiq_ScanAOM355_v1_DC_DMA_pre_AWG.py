...
class allZScan(EnvExperiment):
    ...
    def prepare(self):
        ...
        self.set_dataset("allZScan_Counts.Y_vals",[float(np.nan)], broadcast=True,
                         archive=True)
        # avg data point value's error
        self.set_dataset("allZScan_Counts.Yerr_vals",[float(np.nan)], broadcast=True,
                         archive=True)
        # scan point value
        self.set_dataset("allZScan_Counts.X_vals",[float(np.nan)] , broadcast=True,
                         archive=True)
    ...
    @rpc(flags={"async"})
    def host_push_results(self, scan_val, histpoints, iter): # same name, but not same function as in ndscan
        ...
        if self.CheckThresholding:
            ...
            if iter == 0:
                ...
                # self.mutate_dataset("allZScan_Counts.X_vals", iter, scan_val)
                # self.mutate_dataset("allZScan_Counts.Y_vals", iter, np.mean(histpoints))
                self.append_to_dataset("allZScan_Counts.X_vals", scan_val)
                self.append_to_dataset("allZScan_Counts.Y_vals", np.mean(histpoints))
