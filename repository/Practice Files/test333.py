...
class Simple66(EnvExperiment):
    def build(self):
        ...
        self.setattr_argument("target_frames", NumberValue(default=100, step=1))

        self.target_count = int(self.target_frames)

        self.results = [[0] * 2 for _ in range(self.target_count)]
    ...
    def run(self):
        ...
        np.savetxt(
            ...
            data_matrix,
            fmt="%d",
            header="ROI_0_Count,ROI_1_Count",
            delimiter=",",
        )
        ...
        for i, roi_counts in enumerate(self.results):
            ...
            np.savetxt(
                csv_path,
                np.array([[roi0_val, avg0, roi1_val, avg1]]),
                fmt=["%d", "%.2f", "%d", "%.2f"],
                header="ROI0_Total,ROI0_Avg,ROI1_Total,ROI1_Avg",
                delimiter=",",
            )

            print(
                f"Frame {i + 1:02d}/{self.target_count} saved | "
                f"ROI0 Total: {roi0_val} (Avg: {avg0:.2f}) | "
                f"ROI1 Total: {roi1_val} (Avg: {avg1:.2f})"
            )
    ...
    def run_kernel(self):                           
        ...
        self.grabber0.setup_roi(0, 50, 15, 55, 20)
        self.grabber0.setup_roi(1, 65, 10, 70, 15)
        mask = 0b11
        ...
        while captured_count < self.target_count:
            ...
            print("[Kernel SUCCESS] Grabber data read successfully! ROI0:", roi_buf[0], "ROI1:", roi_buf[1])
            self.core.break_realtime()
