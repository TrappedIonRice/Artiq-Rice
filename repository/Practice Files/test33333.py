...
class Simple633(EnvExperiment):
    def build(self):
        ...
        self.measured_period_ms = 13.0  # 默认占位值，将在 kernel 中更新
    ...
    def run_kernel(self):                           
        ...
        t_rise = self.ttl0.timestamp_mu(now_mu() + self.core.seconds_to_mu(20 * ms))
        if t_rise < 0:
            return

        self.ttl0.gate_falling(20 * ms)
        t_fall = self.ttl0.timestamp_mu(now_mu() + self.core.seconds_to_mu(20 * ms))
        if t_fall < 0:
            return
        ...
        self.measured_period_ms = time_ms
