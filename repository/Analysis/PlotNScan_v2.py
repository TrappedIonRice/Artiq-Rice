...
class PlotNScan2 (ExpFragment):
    
    def build_fragment(self):
        self.fits = {"exponential_decay": exponential_decay,
                "sinusoid": sinusoid,
                "gaussian": gaussian,
                "rabi_flop": rabi_flop,
        ...
        self.setattr_argument("rids", StringParam, "Input list of rids")# = "6707 6708 6709"
        self.setattr_argument("fit_choice",EnumerationValue(self.fits, default="None"))
    
    def run_once(self):

        lst_rids = list(map(int,self.rids.split()))
        print(lst_rids)
        ...
        for ii, rid in enumerate(lst_rids):
            ...
            if not (self.fit_choice=="None"):
                for f in self.fits.keys():

                    if self.fit_choice == f: #"exponential_decay":
                        '''
                        ...
                        '''
                        popt, pcov, x_fit, y_fit = self.fits[f].fit( x_vals_1,
                                                y_vals_1,
                                                 y_err=err_vals_1,
...
PlotNScan2= make_fragment_scan_exp(PlotNScan2)
