...
class awgTriggerer(EnvExperiment):
    def build(self):
        ...
        self.setattr_argument("live_update_scan",
                              BooleanValue(False),
                              group="AWG Scan", tooltip="Enable step-by-step (slower) scan")

        # --- AWG Scan Group ---
        self.setattr_argument("scan_variables_csv",
                              StringValue("AWG.ch1.T0"),
                              group = "AWG Scan",
                              tooltip = "Variable(s) to scan (e.g., 'AWG.ch1.T0' or 'AWG.ch2.T0')")

        # 26/01/08 gt
        # 'enable' for AWG
        self.enableAWG = self.get_argument("Enable AWG", BooleanValue(default=True), group="AWG Scan")
        ...
        # 26/01/07 gt
        # set the scan info as a scannable
        self.setattr_argument("scan_info_V_s_Hz", Scannable(default=RangeScan(0.0, 0.001, 5), global_min=0.0, global_max=10.0,
                                        global_step=1.0e-9))
        self.scan_param_name  = 'Scan_Parameter'
        self.awg_group = 'AWG'
        ...
        # We will store the kernel start/end times here; used to time scan
    ...
    def prepare(self):
        ...
        # 26/01/07 gt: for demonstration of operation of AWG in s2 PC
        self.num_reps = int(self.get_dataset("Repetitions"))
        scan_obj = self.scan_info_V_s_Hz
        self.scan_points = np.array(list(scan_obj)) # extract the scan array from the Scannable
        self.scan_type = type(scan_obj).__name__ # string of scan type name
        self.num_pts = int(len(self.scan_points)) # number of scan pts
        self.scan_variable_list = [v.strip() for v in self.scan_variables_csv.split(',')]
        self.scan_var_name = self.scan_variable_list[0]

        if self.scan_variable_list[0][-2] == 'T':
            self.scan_units = 's'
        elif self.scan_variable_list[0][-2] == 'V':
            self.scan_units = 'V'
        elif self.scan_variable_list[0][-3] == 'f':
            self.scan_units = 'Hz'
        else:
            self.scan_units = ''
        #############################

        # --- 2. Package scan info [needed for both modes (preload/load per scan pt)] ---
        self.scan_info = {
        ...
        relevant_T_vars = [f"AWG.ch{ch}.T0"]#, f"AWG.ch{ch}.T1", f"AWG.ch{ch}.T2", f"AWG.ch{ch}.T3"] # last 3 are relevant if you have multiple waveforms with different durations each
        ...
        try:
            ...
            # print(static_T_values_host_list)
        ...
        for i in range(self.num_pts):
            ...
            for j in range(len(self.is_T_var_scanned_list)):
                ...
                #print(self.static_T_values_list[j])
